    ...
    for (let i = 1; i < values.length; i++) { // Skip header row
      ...
      try {
           ...
           if (isNaN(correctOption) || correctOption < 0 || correctOption > 3 || !Number.isInteger(correctOption)) {
               ...
               // Instead of throwing an error for a single row, log and skip this row
               Logger.log(`Skipping row ${i+1} due to invalid 'correct' value.`);
               continue;
           }
      } catch (e) {
            Logger.log(`Error processing 'correct' value in row ${i+1}: ${e.message}`);
            // Log and skip this row
            Logger.log(`Skipping row ${i+1} due to processing error.`);
            continue;
      }
      ...
      questionData.correct = correctOption; // Explicitly store as number (0-3)
...
function saveQuizResult(name, employeeId, company, score, totalQuestions, userAnswers, language) {
  ...
  try {
    ...
    if (!resultSheet) {
      ...
      // Add headers including the new 'Status' column
      resultSheet.appendRow(['Timestamp', 'Name', 'Employee ID', 'Company', 'Score', 'Total Questions', 'Percentage', 'Language', 'Status']);
      Logger.log(`Sheet '${resultsSheetName}' created with headers.`);
    } else {
       Logger.log(`Sheet '${resultsSheetName}' found.`);
       // Optional: Check if headers exist and add 'Status' if missing
       const headers = resultSheet.getRange(1, 1, 1, resultSheet.getLastColumn()).getValues()[0];
       if (headers.indexOf('Status') === -1) {
         Logger.log("Adding 'Status' header to existing Result sheet.");
         resultSheet.insertColumns(headers.length + 1, 1);
         resultSheet.getRange(1, headers.length + 1).setValue('Status');
       }
    }
    ...
    if (!detailsSheet) {
      ...
      // Add headers
      detailsSheet.appendRow(['Timestamp', 'Name', 'Employee ID', 'Company', 'Question ID', 'Question Number', 'User Answer Index (0-3)', 'Correct Answer Index (0-3)', 'Is Correct', 'Language']);
    ...
    const quizStatus = percentage >= 60 ? '시험 통과' : '재시험 필요';
    Logger.log(`Quiz Status: ${quizStatus}`);


    // Add row to main results sheet
    Logger.log('Appending row to main results sheet...');
    // Append the new Status column value
    resultSheet.appendRow([timestamp, name, employeeId, company, score, totalQuestions, percentage + '%', language, quizStatus]);
    Logger.log('Row appended to main results sheet with status.');

    // Add detailed answer information to details sheet
    Logger.log('Appending rows to detailed results sheet...');
    userAnswers.forEach((answer, index) => {
      // Ensure answer is not null (skipped questions will be null - though with current logic, all should be answered)
      if (answer !== null) {
        ...
        detailsSheet.appendRow([
          ...
          answer.questionNumber,  // Question number in the quiz (1-based)
          answer.userAnswer,      // User answer index (0-3)
          answer.correctAnswer,   // Correct answer index (0-3)
